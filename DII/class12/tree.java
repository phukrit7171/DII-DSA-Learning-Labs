...
public class tree<T> {
    ...
    protected   node<T> _node_found; // สำหรับใช้ใน DFS
    ...
    public void print_tree_v2(node<T> node) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (!n.is_leaf_node()) {
                System.out.print("p:" + n.get_value() + " c: {");
                for (int i = 0; i < n.get_child().size(); i++) {
                    node<T> child_node = n.get_child().get(i);
                    if (child_node != null) {
                        System.out.print(child_node.get_value()+"("+i+"), ");
                        queue.add(child_node);
                    }
                }
                System.out.println("}");
